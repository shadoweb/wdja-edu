      ...
      if(!ii_isnull($tdatabase))
      {
        ...
        if (!ii_isnull($tmpstr))
        {
          ...
          if ((empty($tmprstr) && $type == 'up')||(empty($tmprstr) && $type == 'down')) {
            $tipsnull = '<a rel="prev"><span class="meta-nav"><span class="post-nav">'.$tips.'</span>'.ii_itake('global.lng_config.nomore','lng').'</span></a>';
...
function wdja_cms_init($route)
{
  ...
  if (!(ii_isnull($nport)) && $nport != '80') $nurlpre = $http . $burl.':'.$nport;
